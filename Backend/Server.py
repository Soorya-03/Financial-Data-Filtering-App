...
#code to make call to the api and get app data, replace yourapikey with the actual api key
#apiCall = 'https://financialmodelingprep.com/api/v3/income-statement/AAPL?period=annual&apikey=yourapikey'
...
#extracting the required fields from items recieved from the api.
data = []
for i in range(len(items)):
        requiredData = {'date':items[i]['date'], 'revenue':items[i]['revenue'], 'netIncome':items[i]['netIncome'], 'grossProfit':items[i]['grossProfit'],'eps':items[i]['eps'], 'operatingIncome': items[i]['operatingIncome']}
        data.append(requiredData)
        
app = Flask(__name__)
CORS(app)

#route to print all the unfiltered data
@app.get('/')
def index():
    #get the required fields
    return jsonify(data), 200
    
@app.get('/incomeStatements')
def sortBy():
    #get the required fields
    sortType = request.args.get('sortBy', 'date')
    order = request.args.get('order', 'ascending')
    startYear = request.args.get('startYear')
    endYear = request.args.get('endYear')
    minRevenue = request.args.get('minRevenue')
    maxRevenue = request.args.get('maxRevenue')
    minNetIncome = request.args.get('minNetIncome')
    maxNetIncome = request.args.get('maxNetIncome')
    result = []
    #filtering the data
    for statement in data:
        if int(statement['date'][0:4]) >= int(startYear) and int(statement["date"][0:4]) <= int(endYear):
            if statement['revenue'] >= int(minRevenue) and statement['revenue'] <= int(maxRevenue):
                if statement['netIncome'] >= int(minNetIncome) and statement['netIncome'] <= int(maxNetIncome):
                    result.append(statement)
    #sorting the daya based on parameters.
    result = sorted(result, key=lambda d: d[sortType])
    if order == "descending":
        result.reverse()
    for item in result:
        print(item)
